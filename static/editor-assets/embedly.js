...
(function (global, factory) {
  ...
  FE.PLUGINS.embedly = function (editor) {
    ...
    function _doInit(embedlyInitialized) {
      ...
      editor.events.on('html.processGet', _cleanOnGet); // https://github.com/froala-labs/froala-editor-js-2/issues/1994
      ...
      }, true); // Keep Embedly structure in undo stack.
    ...
    function _init() {
      ...
      if (typeof embedly !== 'undefined') {
        ...
        if (!editor.shared.embedlyLoaded) {
          ...
          editor.shared.embedlyCallbacks.push(_doInit); // Init SCAYT.
    ...
    function _setEmbedlyCardsize(iframe) {
      ...
      $card_parent.attr('contenteditable', false).attr('draggable', true).addClass('fr-draggable').css('height', $card.height()).addClass('fr-draggable');
    ...
    function _initEditPopup() {
      ...
      if (editor.opts.embedlyEditButtons.length > 0) {
        embedly_buttons += "<div class=\"fr-buttons\">\n      ".concat(editor.button.buildList(editor.opts.embedlyEditButtons), "\n      </div>");
        var template = {
    ...
    function _initResizer() {
      ...
      if (!editor.shared.$embedly_resizer) {
        ...
        $embedly_resizer = editor.shared.$embedly_resizer; // Bind mousedown event shared.
    ...
    function _repositionResizer() {
      ...
      $embedly_resizer.css('top', (editor.opts.iframe ? $current_embed.offset().top + iframePaddingTop - 1 + editor.$iframe.position().top : $current_embed.offset().top - editor.$wp.offset().top - 1) + editor.$wp.scrollTop()).css('left', (editor.opts.iframe ? $current_embed.offset().left + iframePaddingLeft - 1 : $current_embed.offset().left - editor.$wp.offset().left - 1) + editor.$wp.scrollLeft()).css('width', $current_embed.outerWidth()).css('height', $current_embed.height()).addClass('fr-active');
    }
    ...
    function _initInsertPopup(delayed) {
      ...
      var template = {
        buttons: buttons,
        url_layer: url_layer // Set the template in the popup.
...
//# sourceMappingURL=embedly.js.map
